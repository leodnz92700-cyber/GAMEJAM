...
def load_map(map_name: str) -> LoadedMap:
    ...
    try:
        ...
        for obj in tree.iter("object"):
            ...
            if rot:
                if obj_id:
                    rotations_by_id[int(obj_id)] = float(rot)
                if obj_name:
                    rotations_by_name[obj_name] = float(rot)
    except Exception:
        pass
    ...
    for layer_name, tiled_objects in tile_map.object_lists.items():
        for tiled_object in tiled_objects:
            ...
            # Retrieve rotation by looking up the Tiled object's ID if possible, fallback to name
            tiled_id = getattr(tiled_object, "id", None)
            rot = rotations_by_id.get(tiled_id) if tiled_id is not None else None
            if rot is None:
